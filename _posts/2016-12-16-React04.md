...
`mount`와 `unmount` 사이에는 여러가지 생명 주기 함수들이 있고, 우리가 앞서 살펴본 `state`와 `props`는 이 함수들과 밀접하게 연관이 있습니다. 위에서도 설명을 했지만, `state`는 컴포넌트의 상태를 나타내는 변수를 담는 객체이고, `props`는 컴포넌트의 속성을 나타내는 변수 값을 담는 객체입니다.
