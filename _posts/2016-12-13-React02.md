...
layout: post
title: React 알아보기 2
summary: React에서 권장되는 JSX문법에 대해서 알아보자
category: React
---
...
`React`에서 `Method`를 생성하기 위해서는 `Class`로 선언하는 것이 아닌, `Component`로 선언해야 합니다. `Component`를 생성하는 방법은 아래와 같습니다.
...
이제 생성한 `Component`에 `Method`를 생성해볼까요? `<button>`을 클릭했을때 "Hello"라고 `alert`를 띄우는 `Method`를 생성해보도록 하겠습니다.
